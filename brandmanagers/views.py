...
def brand_manager_dashboard(request):
    ...
    if request.method == "POST":
        ...
        message = Conversation(sender=user, receiver=recipient, content=message_content)
        message.save()

    # Fetch the list of influencers who have received messages from the brand manager
    influencers = CustomUser.objects.filter(
        received_messages__sender=user, is_influencer=True
    )

    # Retrieve the brand manager's messages
    messages = Conversation.objects.filter(
        Q(sender=user, receiver__in=influencers)
        | Q(sender__in=influencers, receiver=user)
    ).order_by("-timestamp")
...
def content_submission(request):
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            influencer_message = (
                "Your content submission has been received and is awaiting approval."
            )
            send_mail(
                influencer_subject,
                influencer_message,
            ...
            messages.success(
                request, "Content submitted successfully. Awaiting approval."
            )
...
def content_approval(request):
    ...
    if request.method == "POST":
        ...
        submission = get_object_or_404(
            InfluencerContentSubmission.objects.all(), pk=submission_id
        )
        ...
        if action == "approve":
            ...
            if rejection_form.is_valid():
                ...
                messages.error(
                    request, "Invalid rejection form. Please provide a reason."
                )
